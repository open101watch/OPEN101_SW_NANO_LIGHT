...
class FIRMWARE{
  public:
    ...
    //   uint32_t lengthLeft = ESP.getSketchSize(); // Return int
    ...
    //       if (!ESP.flashRead(offset, reinterpret_cast<uint32_t *>(buf.get()), (readBytes + 3) & ~3)) {
    //           return emptyString;
    //       }
...
class App_content_option{
  ...
  public:
    ...
    "SETTINGS", "WATCH FACE", "CALENDER", "STOPWATCH", "CALCULATOR", "GAMES",
    ...
    int page_layouts[50]={0,1,  2,5,  6,11,  12,14,  15,17,  18,23,  24,29,  30,32, 33,33, 34,36,  37,38};
...
class RUN_APP{
  ...
  public:
    ...
    void hibernate(){
      ...
      EEPROM.write(0, bintohex("10000000"));
      EEPROM.commit();
      delay(5000);
      display.clearDisplay();
      display.display();
...
void setup() {
  ...
  if(decimalToBinary(EEPROM.read(0))>=10000000){
    EEPROM.write(0, 0x00);
    EEPROM.commit();
